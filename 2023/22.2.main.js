...
const disintegrate = (blockId) => {
  ...
  while (removed.length) {
    removed.forEach(base => {
      blockList[base].supporting.forEach(supported => {
        checking.add(supported);
      });
    });
    checking.forEach(load => {
      const filtered = [...blockList[load].supportedBy].filter((supportedBy) => {
        return !totalRemoved.includes(supportedBy);
      });
    ...
    totalRemoved = [...totalRemoved, ...next];
    removed = next;
    next = [];
    checking = new Set();
  }
