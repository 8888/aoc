...
class Graph {
  ...
  findCycles(depth = 3) {
    ...
    // find nodes that are in the most cycles
    // map of comp and occurances in a cycle
    const occurances = {};
    uniqueCycles.forEach(cycle => {
      const nodes = cycle.split(',')
      nodes.forEach(node => {
        occurances[node] = occurances[node] ? occurances[node] + 1 : 1;
      })
    })

    // sort around to find the max, then later re sort to
    let cleaned = [];
    for (const [key, value] of Object.entries(occurances)) {
      cleaned.push({comp: key, occurances: value});
    }
    cleaned.sort((a, b) => b.occurances - a.occurances)
    const max = cleaned[0].occurances;
    cleaned = cleaned.filter(x => x.occurances === max);
    cleaned.sort((a, b) => a.comp.localeCompare(b.comp));
    const password = cleaned.reduce((acc, cur) => `${acc + cur.comp},`, '');
